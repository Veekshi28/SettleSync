...
class ExceptionResult:
    ...
    itc_risk_paise: int = 0    # deterministic ₹ exposure for this exception
    itc_risk_label: str = ""   # one-line risk description
...
ITC_RISK_LABELS = {
    "RULE_37A":        "ITC at risk — reverse immediately (Rule 37A)",
    "ITC_TIME_BAR":    "ITC permanently lapsed — cannot be recovered (Sec 16(4))",
    "AMOUNT_MISMATCH": "TDS/adjustment variance — verify Form 16A",
    "TIMING_DIFF":     "No ITC risk — timing difference only",
    "MISSING_ENTRY":   "Unbooked transaction — ITC exposure unquantified",
    "GSTR2B_PENDING":  "No risk — pending GSTR-2B generation",
}

# GSTR2B_PENDING is a pending STATE, not an exception. It must never be treated
# as a compliance risk — excluded from HIGH_RISK_EXCEPTIONS gate, shown as
# "Pending" (not a violation) in the vendor scorecard, and never blocks close.
NON_RISK_CLASSES = {"GSTR2B_PENDING"}


def _compute_itc_risk(
    exception_class: str,
    settlement_amount_paise: Optional[int],
    books_amount_paise: Optional[int],
    gstr_amount_paise: Optional[int],
) -> int:
    """Deterministic ₹ (paise) exposure for a given exception class. No LLM."""
    if exception_class in ("RULE_37A", "ITC_TIME_BAR"):
        return gstr_amount_paise or books_amount_paise or 0
    if exception_class == "AMOUNT_MISMATCH":
        if settlement_amount_paise is not None and books_amount_paise is not None:
            return abs(books_amount_paise - settlement_amount_paise)
        return 0
    if exception_class == "TIMING_DIFF":
        return 0
    if exception_class == "MISSING_ENTRY":
        return settlement_amount_paise or 0
    return 0
...
def _finalize(
    ...
    """Attach the deterministic ITC risk quantification to a classified result."""
    result.itc_risk_paise = _compute_itc_risk(
        result.exception_class, settlement_amount_paise, books_amount_paise, gstr_amount_paise
    )
    result.itc_risk_label = ITC_RISK_LABELS.get(result.exception_class, "")
    return result
...
def classify(
    ...
    # ── Rule 0: GSTR-2B pending window (not an exception — a timing state) ──
    # GSTN generates GSTR-2B on the 14th of the month following the tax period.
    # A current-month invoice checked before the 14th simply hasn't had its
    # GSTR-2B generated yet — this is expected, not a compliance problem.
    if invoice_date is not None:
        same_month = (invoice_date.year == run_date.year and invoice_date.month == run_date.month)
        if same_month and run_date.day < 14:
            return _finalize(ExceptionResult(
                exception_class="GSTR2B_PENDING",
                confidence=0.99,
                explanation=(
                    f"Invoice dated {invoice_date} is from the current period. "
                    "GSTR-2B is generated by GSTN on the 14th of each month — "
                    "this invoice will appear in next month's GSTR-2B. "
                    "No action needed at this time."
                ),
                icai_citation=(
                    "GSTN generates GSTR-2B on the 14th of the month following "
                    "the tax period (Circular No. 173/05/2022-GST). Records with "
                    "current-month invoice dates are in the pending window."
                ),
                resolution_hint=(
                    "Check GSTR-2B after the 14th of next month. "
                    "ITC can be availed once the invoice appears in GSTR-2B."
                ),
            ), settlement_amount_paise, books_amount_paise, gstr_amount_paise)
